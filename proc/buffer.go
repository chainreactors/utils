...
// SetOnWrite installs the write notification under the buffer's lock. Writing
// the field directly races every Write, and a pipe unit has two writers.
func (b *OutputBuffer) SetOnWrite(fn func([]byte)) {
	b.mu.Lock()
	defer b.mu.Unlock()
	b.onWrite = fn
}
